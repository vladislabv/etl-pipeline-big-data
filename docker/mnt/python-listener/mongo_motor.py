...
async def setup_database(client, db):
    async with await client.start_session() as s:
        check = await db.list_collection_names(session=s)
        try:
            if "gateways" not in check:
                v = await read_json_file("/var/data/mongodb/validators/collection_schema_gateways.json")
                _ = await db.create_collection("gateways", validator=v)
            if "tags" not in check:
                v = await read_json_file("/var/data/mongodb/validators/collection_schema_tags.json")
            ...
            if "measures" not in check:
                v = await read_json_file("/var/data/mongodb/validators/collection_schema_measures.json")

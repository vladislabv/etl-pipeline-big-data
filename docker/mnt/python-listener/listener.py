...
async def assure_database_setup(client, db):
    msg = await mongo_motor.setup_database(client, db)
    logger.info('MongoClient answered with %s' % msg)
    print(f"MongoClient answered with {msg}!")
...
async def listen(mongo_client, db, port=1883, host="localhost", **kwargs):
            ...
            async with client.messages() as messages:
                ...
                async for message in messages:
                    if message.topic.matches("gateways/+"):
                        ...
                        await process_measure(message, db['measures'], s)


async def main():
    async with asyncio.TaskGroup() as tg:
        tg.create_task(assure_client_is_up(mongo_client))
        tg.create_task(assure_database_setup(mongo_client, db))
        tg.create_task(
            listen(
...
if __name__ == '__main__':
    ...
    db = mongo_client[os.environ.get('DOCKER_MONGO_DATABASE', 'data')]
    asyncio.run(main())